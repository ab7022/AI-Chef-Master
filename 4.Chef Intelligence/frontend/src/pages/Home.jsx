...
export default function Home() {
  const [rows, setRows] = useState([{ ingredient: "", quantity: "", equipment: "" }]);
  ...
  return (
    <div className={`${colors.backgroundOfBody} ${colors.text} min-h-screen w-screen`}>
      <NavBar toggleSideBar={toggleSideBar} toggleMode={toggleMode} lightMode={lightMode} colos={colors}/>
      <div className="flex flex-row items-center justify-center">
        {sideBarOpen && (
          <SideBar isOpen={sideBarOpen} lightMode={lightMode}/>
        )}
        <div className="flex flex-col w-5/6 items-center justify-center ">
          ...
          <p className={`${colors.textParagraph} text-base md:text-lg  mt-4 mb-0 text-center font-semibold max-w-2xl`}>
          ...
          <p className={`${colors.textParagraph} text-lg mb-12 md:mb-4 text-center hidden md:block font-semibold`}>
            Start your culinary adventure today with AI Chef Master!
          </p>
          <div className={`${colors.backgroundOfDiv} shadow-xl rounded-xl p-3 justify-center md:mt-2 mt-12`}>
            {rows.map((row, index) => (
              <div key={index} className="gap-2 mb-2 px-2">
                <div className="flex-col flex gap-3 md:flex-row mt-4">
                  <input
                    ...
                    onChange={(event) => handleInputChangeOfIngredient(index, event)}
                  ...
                  <input
                    ...
                    name="equipment"
                    value={row.equipment}
                    onChange={(event) => handleInputChangeOfEquipment(index, event)}
            ...
            <div className="flex justify-center gap-2 mt-4 md:mt-6 items-center align-middle">
              <button className={`py-2 px-4 ${colors.buttonBackground} ${colors.buttonHoverBackground} ${colors.buttonTextColor} ${colors.buttonHoverTextColor} font-bold rounded focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50`} onClick={handleAddRow}>
                Add More
              </button>
            </div>
          </div>
          <button className={`${colors.button} ${colors.buttonHoverBackground} ${colors.buttonTextColor} ${colors.buttonHoverTextColor} text-white font-bold py-3 px-4 rounded-xl focus:outline-none focus:ring-2 mt-12 focus:ring-gray-500 focus:ring-opacity-50 mb-4 md:mb-12 ${colors.buttonHoverTextColor}`}>
            Start To Process
          </button>
        </div>
      </div>
