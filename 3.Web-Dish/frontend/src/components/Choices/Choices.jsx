...
const Choices = () => {
  ...
  return (
    <div className='text-zinc-900 bg-[#00544f] flex flex-col justify-center items-center py-8'>
      ...
      <div
        ref={ref}
        className={`text-white  grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-6 xl:grid-cols-6 gap-2 sm:gap-8 md:gap-16 lg:gap-6 xl:gap-16 justify-around`}
      >
        {ChoicesData.map((item, index) => (
          <Card1 key={index} title={item.title} imageUrl={item.imageUrl} animate={animate} />
        ))}
      </div>
