...
function Vegetables({ dishType }) {
    const filteredData = dishType ? VegetablesData.filter(dish => dish.dishType === dishType) : VegetablesData;
  return (
    <div className="bg-[#f7f3cd]">
    <div className='flex justify-end mt-12 mb-6 mr-8'>
        <button onClick={()=> window.scrollTo(0, 0)}>
            <Link to='/VegetablesSeeMore' className=' text-end font-bold text-lg sm:text-xl md:text-2xl'>See More</Link>
        </button>
    </div>
    <Marquee> 
